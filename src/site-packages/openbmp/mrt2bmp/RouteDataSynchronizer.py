    ...
    def run(self):
        ...
        try:
            ...
            while True:
                ...
                time_difference_minutes = (self.current_timestamp - datetime.datetime.strptime(last_modified, '%a, %d %b %Y %H:%M:%S GMT'))\
                                              .total_seconds() / 60

                if not self.__checkFileMetadataInDownloadTrack(df[1], key, etag, last_modified) and time_difference_minutes >= 3:
...
class RouteDataSynchronizer(Process):
    ...
    def __createDownloaderThread(self, router_name):
        ...
        t = Downloader_Thread(self.download_queue, self.route_view_dir_path, router_name)
        t.setDaemon(True)
        t.start()
    ...
    def getListOfRouters(web_address):
        ...
        try:
            response = urllib2.urlopen(web_address)
            html = response.read()
