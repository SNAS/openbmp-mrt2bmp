        ...
        for m in mp:
            ...
            if MRT_TYPES[m['mrt_header']['type']] == 'TABLE_DUMP_V2' and \
                ...
                for e in m['mrt_entry']['rib_entries']:
                    ...
                    qm = Q_Message(Q_Message.BMP_MESSAGE, bmp_common_header + bmp_per_peer_header +
                                   bgp_common_header + bgp_update_message)
    ...
    def processUpdateFile(self):
        ...
        if self._isProcessable:
            ...
            for m in mp:
                ...
                # Put the message in the queue.
                qm = Q_Message(Q_Message.BMP_MESSAGE, common_header + per_peer_header + raw_bgp_message)
...
class RouterProcessor(BMP_Helper):
            ...
            if os.path.isfile(path):
                ...
                for m in mp:
                    ...
                    if m['mrt_header']['type'] == 13 and m['mrt_header']['subtype'] == 1:
                        ...
                        print "Collector ID: " + self._collector_id
                        break
    ...
    def __sendTerminationMessage(self):
        ...
        qm = Q_Message(Q_Message.BMP_MESSAGE, common_header + reason_tlv)
        ...
        if self._isToProcess:
            for f in self._listOfRibAndUpdateFiles:
                ...
                moveFileToTempDirectory(os.path.join(self._directory_path, self._router_name, f[1]),
                                        os.path.join(self._processed_directory_path, self._router_name, f[1]))

        else:
            self.LOG.error("Data of %s cannot be processed..." % self._router_name)

        self.__sendTerminationMessage()
    ...
    def run(self):
        ...
        try:
            ...
            for router in listOfRouters:
                ...
                if router.startswith('r') and not self.stopped():
